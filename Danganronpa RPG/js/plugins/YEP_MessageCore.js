...
//=============================================================================
 /*:
 * @plugindesc Agrega más características a la ventana de mensajes para
 * personalizar la forma en que se ven mensajes y funciones.
 * @author Yanfly Engine Plugins - (es) v1.07
 *
 * @param ---General---
 * @default
 *
 * @param Default Rows
 * @desc Este es el numero predeterminado de líneas que el cuadro
 * de mensajes va a tener.  Por defecto: 4
 * @default 4
 *
 * @param Default Width
 * @desc Este es el ancho estándar para el cuadro de mensajes en
 * pixeles.  Por defecto: Graphics.boxWidth
 * @default Graphics.boxWidth
 *
 * @param Face Indent
 * @desc Si usa una face gráfica, establece como el texto va ser
 * sangrado.  Por defecto: Window_Base._faceWidth + 24
 * @default Window_Base._faceWidth + 24
 *
 * @param Fast Forward
 * @desc Habilita un botón de avance rápido para avanzar hacia 
 * adelante. Si no desea esto, cambie a 'false' en su lugar.
 * @default Input.isPressed('pagedown')
 *
 * @param Word Wrapping
 * @desc Esta opción activa o desactiva el cubrimiento de palabras
 ...
 * @desc Inserta un espacio con saltos de línea manuales ?
 * NO - false     SI - true
 * @default false
 * 
 * @param ---Font(Fuente)---
 ...
 *
 * @param Font Size
 * @desc Este es el tamaño de fuente predeterminada utilizada para 
 ...
 * @desc Este es el tamaño máximo alcanzado por \{.
 * Por defecto: 96
 * @default 96
 *
 * @param Font Changed Min
 * @desc Este es el tamaño mínimo alcanzado por \{.
 * Por defecto: 12
 * @default 12
 *
 ...
 * @desc Este es el buffer para la ubicación X de la Caja de Nombre.
 * @default -28
 *
 * @param Name Box Buffer Y
 * @desc Este es el buffer para la ubicación Y de la Caja de Nombre.
 * @default 0
 *
 ...
 * Se usa para establecer automáticamente los colores.
 * @default \c[6]
 *
 * @help
 * ============================================================================
 ...
 * de mensaje, no estaría mal agregar algunas características mas, como  
 * ventana de nombres, convertir el código de textos para escribir los iconos 
 * y/o *[English > and/or]* los nombres de los items, armas, armaduras, y* de 
 * una manera más rápida. Este script también da al desarrollador la 
 * habilidad de ajustar el tamaño de la ventana de mensajes durante el juego,
 * asi como darle una fuente separada, y darle al jugador la característica 
 * de poder realizar un avance rápido del texto.
 *
 * ============================================================================
 * Word Wrapping (Cubrimiento de Palabras)
 * ============================================================================
 *
 * Cubrir palabra ahora es posible a través del sistema de mensajes. Puedes 
 * activar y desactivar Cubrir Palabra usando los comandos de Plugin. durante 
 * el uso de Cubrir Palabra, si la palabra se extiende más allá del área de 
 * mensajes de la ventana, pasara automáticamente a la siguiente línea.
 * dicho esto, Cubrir palabra inhabilitara los saltos de línea del editor
 * y requerirá que utilice los proporcionados por el plugin:
 *
 * <br> o <line break> es el código de texto para aplicar una rotura de línea.
 * Utilice esto antes o después de la parte en la que desea iniciar una
 * nueva línea.
 *
 ...
 * como la descripciones de los artículos, inserta <WordWrap>
 * en el comienzo del texto para activarlo.
 *
 * ============================================================================
 * Text Codes (Código de Textos)
 * ============================================================================
 *
 * En el uso de ciertos códigos de textos en tus mensajes, Usted puede hacer 
 * que el juego los sustituya con los siguientes:
 *
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 *
 * Código de Texto    Función
 ...
 *   \{          Aumenta el tamaño del texto en un paso.
 *   \}          Reduce el tamaño del texto en un paso.
 *   \\          Reemplaza con el carácter de barra invertida.
 *   \$          Abre la ventana de oro.
 *   \.          Espera 1/4th de segundo.
 *   \|          Espera 1 un segundo.
 *   \!          Espera la entrada de botón.
 *   \>          Visualizar todo el resto del texto en la misma línea a la vez.
 *   \<          Cancela el efecto que muestra el texto a la vez.
 *   \^          No espera la entrada de datos después de visualizar el texto.
 *
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 *
 *  Esperar:    Efecto:
 ...
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 *
 *  Salto de línea  		Efecto:
 *    <br>     - Si utiliza el modo de ajuste de línea,
 *               esto provocara un salto de línea.
 *
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 *
 *  posición:   Efecto:
 *    \px[x]    - Establece posición X para el texto X.
 *    \py[x]    - Establece posición Y para el texto Y.
 *
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 *
 *  Esquema:    Efecto:
 ...
 *    \fs[x]    - Cambia el tamaño de fuente a X.
 ...
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 *
 * Y estos son los códigos de texto agregados en este script. tenga en mente 
 * que algunas clases de código de texto solo funcionan para la ventana 
 * de mensajes. De lo contrario, trabaja para las descripciones de ayuda,
 * biografías de personaje, y otros.
 *
 * ============================================================================
 * Comandos de Plugin
 * ============================================================================
 *
 * Los siguientes son unos comandos de plugin que usted puede usar a través del
 * editor de eventos para cambiar varios aspectos sobre el sistema de mensaje.
 *
 * Comando de plugin
 *   MessageRows 6          = Cambia las líneas de mensajes mostradas a 6. 
 *                            Si estas usando continuamente eventos de 
 *                            Mostrar Texto, esto continuara mostrando los 
 *                            siguientes textos de líneas hasta llegar al
 *                            límite de línea. Cualquier cosa después de eso
 *                            es cortada hasta el siguiente mensaje comenzar,
 *                            para evitar un superposición por accidente.
 *
 ...
 *                            extiende más allá del tamaño de la ventana, 
 *                            se moverá automáticamente a la siguiente línea.
 ...
 *                            línea serán automáticos en los puntos donde se 
 *                            inicie una nueva línea en el editor.
 *
 * ============================================================================
 * Changelog
 * ============================================================================
